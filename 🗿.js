...
let soundList = []
fetch("./sounds.json?v=2").then(x => x.json()).then(list => {
    soundList = list
    list.forEach(x => {
        let imageLink = (!x.emoji && x.id.match(/[a-z0-9]/i)) ? `icons/${x.id}.png` : `https://twemoji.maxcdn.com/v/14.0.2/svg/${(x.emoji || x.id).codePointAt(0).toString(16)}.svg`
        $('#icons').append(`<div class="sound" soundName="${x.name}" soundOrigin="${x.source || ""}" sound="sounds/${x.id}.wav" str="${x.id}"><img alt="${x.name}" src="${imageLink}"></div>`)
        lastGroup = x.group
    })
})
...
$(document).on('click contextmenu', '.iconbox', function () { return false })

$(document).on('click contextmenu', '#icons div', function (event) {
    cancel()
...
$(document).on('click contextmenu', '#actions div', function (event) {
    cancel()
...
$(document).on('click', '#sequence div', function () {
    ...
    if (shiftHeld) {
        ...
        playSound($(this).attr("sound"), { pitch: getPitch($(this)), stopPrevious: true })
    }
    else $(this).remove()
...
$(document).on('contextmenu', '#sequence div', function () {
    ...
    if (ctrlHeld) return
        ...
        playSound(snd, { pitch: getPitch($(this)), stopPrevious: true})
    }
...
$(document).on('wheel touchmove', '#sequence div', function(event) {
    ...
    if (el.attr("sound") && !el.attr("str").startsWith("_")) {
        shift += ((downward ? -1 : 1) * (shiftHeld ? 6 : altHeld ? 0.2 : 1))
        ...
        el.attr("pitch", shift)
        playSound(el.attr("sound"), { pitch: semitonesToPercent(shift), stopPrevious: true })
    }
...
$(document).on('change', '.settingBox', function() {
